...
PC, 태블릿, 모바일까지 기기 환경에 맞춰 이미지와 텍스트 레이아웃이 자연스럽게 변환되도록 구축하여 모바일 접근성을 크게 향상시켰습니다.
### SEO 최적화
${category} 관련 핵심 키워드 검색 시 상위 노출이 유리하도록 체계적인 헤딩(H1~H3) 태그를 적용하고 가벼운 로딩 속도를 구현했습니다.`;
...
const mdToBlocks = (mdString) => {
  ...
  for (const line of lines) {
    if (line.startsWith('# ')) {
      ...
      blocks.push({ heading_3: { rich_text: [{ text: { content: line.substring(4) } }] } });
    } else {
      // 빈 줄도 포함하여 paragraph 생성
      blocks.push({ paragraph: { rich_text: line.trim() !== '' ? [{ text: { content: line } }] : [] } });
    }
  }
  return blocks;
};
...
async function updateAllSeo() {
  ...
  for (const page of data.results) {
    ...
    if (title.toLowerCase().includes('yg') || title.includes('성형외과')) {
